...
import pytest
import os
from kmcpy.simulation_condition import SimulationConfig, create_nasicon_config
...
class TestNASICONbulk(unittest.TestCase):
    ...
    def test_kmc_main_function_modernized(self):
        ...
        try:
            ...
            kmc_tracker = kmc.run_with_config(config)

            print(f"Modern SimulationCondition results: {kmc_tracker.return_current_info()}")
    ...
    def test_simulation_condition_with_nasicon(self):
        ...
        self.assertTrue(hasattr(KMC, 'from_simulation_config'))
        self.assertTrue(hasattr(KMC, 'run_simulation'))
        self.assertTrue(hasattr(KMC, 'run_with_config'))
        ...
        try:
            ...
            print(f"⚠ KMC instance creation failed (expected with test files): {type(e).__name__}")
    ...
    def test_simulation_condition_parameter_studies(self):
        ...
        self.assertEqual(len(set(temps_and_freqs)), 4)  # All combinations should be unique
    ...
    def test_kmc_simulation_condition_vs_inputset(self):
        ...
        try:
            ...
            kmc_simulation = KMC.from_simulation_config(config)
    ...
    def test_simulation_condition_parameter_matching(self):
        ...
        try:
            ...
            print(f"✓ Temperature: {inputset.temperature} == {inputset_from_config.temperature}")
            ...
            self.assertEqual(inputset.elem_hop_distance, inputset_from_config.elem_hop_distance)
            print(f"✓ Elementary hop distance: {inputset.elem_hop_distance}")
            ...
            # Mobile ion species
            self.assertEqual(inputset.mobile_ion_specie, inputset_from_config.mobile_ion_specie)
            print(f"✓ Mobile ion species: {inputset.mobile_ion_specie}")
            
            # Supercell shape
            self.assertEqual(inputset.supercell_shape, inputset_from_config.supercell_shape)
            ...
            # Immutable sites
            self.assertEqual(inputset.immutable_sites, inputset_from_config.immutable_sites)
            print(f"✓ Immutable sites: {inputset.immutable_sites}")
            
            # Convert to primitive cell
            self.assertEqual(inputset.convert_to_primitive_cell, inputset_from_config.convert_to_primitive_cell)
            print(f"✓ Convert to primitive cell: {inputset.convert_to_primitive_cell}")
    ...
    def test_kmc_simulation_condition_workflow(self):
        ...
        try:
            ...
            # Test 2: Run simulation using run_with_config (recommended approach)
            print("Running KMC simulation using run_with_config...")
            ...
            print("✅ SimulationCondition results match expected reference values!")
            ...
            high_temp_config = config.copy_with_changes(
                temperature=400.0,
                name="NASICON_HighTemp_Test"
            )
            
            self.assertEqual(high_temp_config.temperature, 400.0)
            self.assertEqual(high_temp_config.name, "NASICON_HighTemp_Test")
            self.assertEqual(high_temp_config.attempt_frequency, config.attempt_frequency)  # Should be unchanged
            ...
            print("✅ SimulationCondition system is working correctly and produces expected results!")
