...
def test_enumeration_infers_fixed_sites_from_mapping():
    ...
    assert [result.label for result in results] == ["1:Si", "1:P"]
    assert all(result.local_site_indices == (0, 1) for result in results)
    assert all(result.variable_site_indices == (1,) for result in results)
...
def test_neb_endpoint_pair_rejects_invalid_hops():
    ...
    with pytest.raises(IndexError, match="outside the active-site range"):
