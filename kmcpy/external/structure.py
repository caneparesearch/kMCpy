...
class StructureKMCpy(Structure):
    @classmethod
    def from_cif(
        cls, filename, primitive=False, sort=False, merge_tol=0.0
    ) -> "StructureKMCpy":
        ...
        Returns:
            kmcpy.external.structure.StructureKMCpy: a structure generated by the cif with all information you need.
        """
    ...
    def from_sites(
        ...
        for k, v in props.items():
            if any(vv is None for vv in v):
                warnings.warn(
                    f"Not all sites have property {k}. Missing values are set to None."
                )
        return cls(
            lattice,
    ...
    def find_site_by_wyckoff_sequence_and_label(self, wyckoff_sequence=0, label="Na2"):
        ...
        if "wyckoff_sequence" not in self.site_properties:
            raise ValueError(
                "wyckoff_sequence not found in site properties.make sure that this structure is generated by kmcpy.StructureKMCpy.from_cif() function!"
            )
        for i in self._sites:
            if (
                i.properties["wyckoff_sequence"] == wyckoff_sequence
                and i.properties["label"] == label
            ):
                return i
        raise ValueError(
            "find_site_by_wyckoff_sequence_and_label:not found. with input parameter: Wyckoff sequence=",
            wyckoff_sequence,
            " label=",
            label,
        )
    ...
    def find_site_by_wyckoff_sequence_label_and_supercell(
        self, wyckoff_sequence=0, label="Na2", supercell=(0, 1, 1), return_index=True
    ):
        ...
        if "wyckoff_sequence" not in self.site_properties:
            raise ValueError(
                "wyckoff_sequence not found in site properties.make sure that this structure is generated by kmcpy.external.structure.StructureKMCpy.from_cif() function!"
            )
        if return_index:
            for i in range(0, len(self._sites)):
                if (
                    self._sites[i].properties["wyckoff_sequence"] == wyckoff_sequence
                    and self._sites[i].properties["label"] == label
                    and self._sites[i].properties["supercell"] == supercell
                ):
                    return i
        else:
            for i in self._sites:
                if (
                    i.properties["wyckoff_sequence"] == wyckoff_sequence
                    and i.properties["label"] == label
                    and i.properties["supercell"] == supercell
                ):
                    return i
        raise ValueError(
            "find_site_by_wyckoff_sequence_label_and_supercell:not found. with input parameter: Wyckoff sequence=",
            wyckoff_sequence,
            " label=",
            label,
            " supercell",
    ...
    def make_kmc_supercell(
        self, scaling_matrix=(1, 2, 3), to_unit_cell=True
    ) -> "StructureKMCpy":
        ...
        Returns:
            Structure: kmcpy.external.structure.StructureKMCpy
        """
        ...
        fractional_translations_for_supercell = lattice_points_in_supercell(
            scale_matrix
        )
        ...
        for site in self:
            ...
            for (
                fractional_translation_for_supercell
            ) in fractional_translations_for_supercell:

                this_properties = site.properties.copy()
                this_properties["supercell"] = tuple(
                    np.rint(
                        np.dot(fractional_translation_for_supercell, scale_matrix)
                    ).astype(int)
                )
                ...
                s = PeriodicSite(
                    site.species,
                    site.coords
                    + new_lattice.get_cartesian_coords(
                        fractional_translation_for_supercell
                    ),
                    new_lattice,
        ...
        supercell = StructureKMCpy.from_sites(
            new_sites, charge=new_charge, to_unit_cell=to_unit_cell
        )
    ...
    def kmc_build_dict(self, skip_check=True):
        """
        Build a dictionary mapping unique site identifiers to their indices in the structure.

        The unique identifier for each site is generated using the `site_index_vector` method,
        which combines the 'supercell', 'label', and 'local_index' properties of each site.

        Args:
            skip_check (bool, optional): If True (default), skips checking for duplicate site
            identifiers and builds the dictionary directly. If False, checks for duplicate
            identifiers and raises a KeyError if any are found.

        Returns:
            dict: A dictionary mapping the unique site identifier (as returned by `site_index_vector`)
            to the site index.

        Raises:
            KeyError: If any of the required site properties ('supercell', 'label', 'local_index')
            are missing, or if `skip_check` is False and duplicate site identifiers are found.
        """
        
        for prerequisite in ["supercell", "label", "local_index"]:
            if prerequisite not in self.site_properties:
                raise KeyError(
                    "make sure the structure is generated by structure.from_cif().make_kmc_supercell!"
                )
        ...
        if skip_check:
            ...
            for site_index in range(0, len(self._sites)):
                indices_dict_from_identifier[
                    self.site_index_vector(
                        supercell=self._sites[site_index].properties["supercell"],
                        label=self._sites[site_index].properties["label"],
                        local_index=self._sites[site_index].properties["local_index"],
                    )
                ] = site_index
        else:
            for site_index in range(0, len(self._sites)):
                tmp_key = self.site_index_vector(
                    supercell=self._sites[site_index].properties["supercell"],
                    label=self._sites[site_index].properties["label"],
                    local_index=self._sites[site_index].properties["local_index"],
                )
                ...
                if tmp_key in indices_dict_from_identifier:
                    raise KeyError(
                        "duplicate sites identifies by kmc_build_dict function. This shouldn't happen. Please check this site: ",
                        self._sites.properties,
                    )
