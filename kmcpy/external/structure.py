...
class StructureKMCpy(Structure):
    ...
    def from_cif(
        ...
        Returns:
            kmcpy.external.StructureKMCpy: a structure generated by the cif with all information you need.
        """
        # modified version of from file, only read the cif
        filename = str(filename)

        from kmcpy.external import CifParserKMCpy
    ...
    def find_site_by_wyckoff_sequence_label_and_supercell(
        ...
        if "wyckoff_sequence" not in self.site_properties:
            raise ValueError(
                "wyckoff_sequence not found in site properties.make sure that this structure is generated by kmcpy.external.StructureKMCpy.from_cif() function!"
            )
        if return_index:
    ...
    def make_kmc_supercell(
        ...
        Returns:
            Structure: kmcpy.external.StructureKMCpy
        """
    ...
    def kmc_build_dict(self, skip_check=True):
        ...
        if skip_check:
            ...
            for site_index in range(0, len(self._sites)):
                tmp_key = self.kmc_info_to_tuple(
                    supercell=self._sites[site_index].properties["supercell"],
                    label=self._sites[site_index].properties["label"],
                    wyckoff_sequence=self._sites[site_index].properties[
                        "wyckoff_sequence"
